...
ax.set_xlabel('ワイン銘柄番号')

# ax.yaxis.tick_right()
ax.yaxis.set_ticks_position('both')
ax.yaxis.set_tick_params(length=0)

plt.colorbar(im, ax=ax, label='相対スコア', pad=0.01)
ax.set_yticks(range(df.shape[1]))
ax.set_yticklabels(df.columns, fontsize=14)

# ax.set_xlabel('Samples')
# ax.set_ylabel('Features')
# plt.title('Heatmap of wine data (z-score normalized)', pad=100)
# plt.tight_layout()
plt.savefig('4_1_4_heatmap.png', bbox_inches='tight', pad_inches=0.1, dpi=300)
plt.savefig('4_1_4_heatmap.svg', bbox_inches='tight', pad_inches=0.1, dpi=300)
