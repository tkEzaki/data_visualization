...
# 共著データのネットワークを円状に描画
plt.sca(axes[0, 0])
nx.draw_circular(collaboration_network, with_labels=True, width=2, edge_color='black', node_color='gray', font_color='white',font_size=15)
# axes[0, 0].set_title("Collaboration Network - Circular Layout")

# 共著データのネットワークをspring layoutで描画
plt.sca(axes[0, 1])
nx.draw_spring(collaboration_network, with_labels=True, width=2, edge_color='black', node_color='gray', font_color='white',font_size=15)
...
nx.draw_circular(research_scores_network, with_labels=True, width=edge_widths, edge_color=edge_colors, edge_cmap=plt.cm.jet, node_color='gray', font_color='white',font_size=15, edge_vmin=0, edge_vmax=1)
...
edge_widths = [research_scores_network[u][v]['weight'] * 5 for u, v in research_scores_network.edges()]
edge_colors = [research_scores_network[u][v]['weight'] for u, v in research_scores_network.edges()]
nx.draw_spring(research_scores_network, with_labels=True, width=edge_widths, edge_color=edge_colors, edge_cmap=plt.cm.jet, node_color='gray', font_color='white',font_size=15, edge_vmin=0, edge_vmax=1)
# axes[1, 1].set_title("Research Scores Network - Spring Layout")

plt.tight_layout()
# spaceを調整
plt.subplots_adjust(wspace=0.2, hspace=0.2)
plt.savefig("4_2_2_network_visualization.png", dpi=300)
plt.savefig("4_2_2_network_visualization.svg", dpi=300)
