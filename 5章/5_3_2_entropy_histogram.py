...
plt.subplot(1, 3, 1)
plt.hist(uniform_samples, bins=bins, color='skyblue', edgecolor='k')
plt.title(f'一様分布\nエントロピー: {uniform_entropy:.2f}')
plt.ylabel('頻度')

plt.subplot(1, 3, 2)
plt.hist(normal_samples, bins=bins, color='salmon', edgecolor='k')
plt.title(f'正規分布\nエントロピー: {normal_entropy:.2f}')

plt.subplot(1, 3, 3)
plt.hist(gmm_samples, bins=bins, color='lightgreen', edgecolor='k')
plt.title(f'2つの正規分布の混合\nエントロピー: {gmm_entropy:.2f}')


plt.tight_layout()
plt.savefig('5_3_2_entropy_histogram.png', dpi=300)
plt.savefig('5_3_2_entropy_histogram.svg', dpi=300)
