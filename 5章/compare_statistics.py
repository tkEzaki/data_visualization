...
for idx, distribution in enumerate(['normal', 'lognormal']):
    ...
    for sample_size in sample_sizes:
        ...
        # 最大値と95パーセンタイルのプロット
        sns.swarmplot(x=[f'N={sample_size}\n最大値'] * 100 + [f'N={sample_size}\n95\nパーセンタイル'] * 100, y=max_values + percentiles_95, ax=axes[2, idx])
        
# タイトルとラベルの設定
for i in range(3):
    axes[i, 0].set_ylabel(['平均値・中央値', '標準偏差・四分位範囲', '最大値・95パーセンタイル点'][i])
    axes[i, 1].set_ylabel('')
...
plt.savefig('compare_statistics.png', dpi=300)
