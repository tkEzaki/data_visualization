...
plt.tight_layout()
plt.savefig('7_1_1_janken_time_series.png', dpi=300)
plt.savefig('7_1_1_janken_time_series.svg', dpi=300)
...
def plot_transition_graph(ax, prob_matrix, title):
    ...
    for edge, label in labels.items():
        ...
        if p1[0] != p2[0]:
            ...
            shift = 0.1
            # ラベルの位置をずらす
            ax.text(p[0] - n[0] * shift, p[1] - n[1] * shift, label, fontsize=14, ha='center', va='center')
        else:
            shift = 0.15
            ax.text(p[0] + shift, p[1] + shift, label, fontsize=14, ha='center', va='center')
...
plt.tight_layout()
plt.savefig('7_1_1_janken_transition_analysis.png', dpi=300)
plt.savefig('7_1_1_janken_transition_analysis.svg', dpi=300)
